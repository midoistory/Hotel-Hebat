...
@section('title-web', 'Edit Fasilitas Ruangan')

@section('title-content', 'Edit Fasilitas Ruangan')

@section('content')
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form action="{{ route('admin.roomfacility.update', $roomFacility->id) }}" method="POST"
                ...
                <select name="room_type_id"
                    ...
                    @foreach ($roomTypes as $roomType)
                        <option value="{{ $roomType->id }}"
                            {{ old('room_type_id', $roomFacility->room_type_id) == $roomType->id ? 'selected' : '' }}
            ...
            @foreach ($facilities as $facility)
                <label class="block mt-4 text-sm">
                    ...
                    <textarea name="{{ $facility['name'] }}"
                        ...
                        rows="2" placeholder="{{ $facility['placeholder'] }}">{{ old($facility['name'], $roomFacility->{$facility['name']}) }}</textarea>
